...
hw_ctx::hw_ctx(device& device, const std::vector<uint8_t>& pdi,
  ...
  shim_amdxdna_bo_flags f = {};
  f.flags = AMDXDNA_BO_FLAGS_CACHEABLE;
...
int hw_ctx::alloc_bo(size_t size, shim_amdxdna_bo_flags flags,
                     std::unique_ptr<bo>* out_bo) {
  // Debug buffer is specific to one context.
  if (flags.use == AMDXDNA_BO_USE_DEBUG)
...
void hw_ctx::init_log_buf() {
  ...
  shim_amdxdna_bo_flags f;
  f.flags = AMDXDNA_BO_FLAGS_EXECBUF;
