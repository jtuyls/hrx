 ...
 *
 * Header file *ert.h* defines data structures used by Emebdded Runtime (ERT)
 * and XRT xclExecBuf() API.
 */
...
#define to_cfg_pkg(pkg) ((struct ert_configure_cmd*)(pkg))
#define to_start_krnl_pkg(pkg) ((struct ert_start_kernel_cmd*)(pkg))
#define to_copybo_pkg(pkg) ((struct ert_start_copybo_cmd*)(pkg))
#define to_cfg_sk_pkg(pkg) ((struct ert_configure_sk_cmd*)(pkg))
#define to_init_krnl_pkg(pkg) ((struct ert_init_kernel_cmd*)(pkg))
#define to_validate_pkg(pkg) ((struct ert_validate_cmd*)(pkg))
#define to_abort_pkg(pkg) ((struct ert_abort_cmd*)(pkg))
...
struct ert_npu_data {
  ...
  uint32_t instruction_buffer_size; /* size of buffer in bytes */
  uint32_t
      instruction_prop_count; /* WORD length of following properties nv pairs */
};

/**
 * struct ert_npu_preempt_data - interpretation of data payload for
 * ERT_START_NPU_PREEMPT
 *
 * @instruction_buffer:       address of instruction buffer
...
struct ert_npu_preempt_data {
  ...
  uint32_t instruction_prop_count;  /* DWORD length of following properties nv
                                       pairs */
};
 ...
 * @ERT_START_COPYBO:      start KDMA CU or P2P, may be converted to
 * ERT_START_CU before cmd reach to scheduler, short-term hack
...
#define ERT_INTC_CU_96_127_IAR \
  (ERT_INTC_CU_96_127_ADDR + 0x0C) /* acknowledge */
#define ERT_INTC_CU_96_127_MER \
  (ERT_INTC_CU_96_127_ADDR + 0x1C) /* master enable */
 ...
 */
static inline void ert_fill_copybo_cmd(struct ert_start_copybo_cmd* pkt,
                                       uint32_t src_bo, uint32_t dst_bo,
...
static inline uint64_t ert_copybo_src_offset(struct ert_start_copybo_cmd* pkt) {
  return (uint64_t)pkt->src_addr_hi << 32 | pkt->src_addr_lo;
}
static inline uint64_t ert_copybo_dst_offset(struct ert_start_copybo_cmd* pkt) {
  return (uint64_t)pkt->dst_addr_hi << 32 | pkt->dst_addr_lo;
}
static inline uint64_t ert_copybo_size(struct ert_start_copybo_cmd* pkt) {
  return pkt->size;
}

static inline bool ert_valid_opcode(struct ert_packet* pkt) {
  ...
  switch (pkt->opcode) {
    ...
    case ERT_START_DPU:
       ...
       */
      valid =
          (skcmd->count >= 1 + skcmd->extra_cu_masks +
                               sizeof(struct ert_dpu_data) / sizeof(uint32_t));
      break;
    case ERT_CMD_CHAIN:
      ccdata = (struct ert_cmd_chain_data*)pkt->data;
      /* header count must match number of commands in payload */
      valid = (pkt->count == (ccdata->command_count * sizeof(uint64_t) +
                              sizeof(struct ert_cmd_chain_data)) /
                                 sizeof(uint32_t));
      break;
    case ERT_START_NPU:
      skcmd = to_start_krnl_pkg(pkt);
    ...
    case ERT_SK_CONFIG:
      ...
      valid = (cscmd->count ==
               sizeof(struct config_sk_image) * cscmd->num_image / 4 + 1);
      break;
    case ERT_CLK_CALIB:
...
static inline uint64_t get_ert_packet_size_bytes(struct ert_packet* pkt) {
  // header plus payload
  return sizeof(pkt->header) + pkt->count * sizeof(uint32_t);
}

static inline struct ert_dpu_data* get_ert_dpu_data(
    struct ert_start_kernel_cmd* pkt) {
  if (pkt->opcode != ERT_START_DPU) return NULL;

  // past extra cu_masks embedded in the packet data
  return (struct ert_dpu_data*)(pkt->data + pkt->extra_cu_masks);
}

static inline struct ert_dpu_data* get_ert_dpu_data_next(
    struct ert_dpu_data* dpu_data) {
  if (dpu_data->chained == 0) return NULL;

  return dpu_data + 1;
}

static inline struct ert_cmd_chain_data* get_ert_cmd_chain_data(
    struct ert_packet* pkt) {
  if (pkt->opcode != ERT_CMD_CHAIN) return NULL;

  return (struct ert_cmd_chain_data*)pkt->data;
}

static inline struct ert_npu_data* get_ert_npu_data(
    struct ert_start_kernel_cmd* pkt) {
  if (pkt->opcode != ERT_START_NPU) return NULL;

  // past extra cu_masks embedded in the packet data
  return (struct ert_npu_data*)(pkt->data + pkt->extra_cu_masks);
}

static inline struct ert_npu_preempt_data* get_ert_npu_preempt_data(
    struct ert_start_kernel_cmd* pkt) {
  if (pkt->opcode != ERT_START_NPU_PREEMPT) return NULL;
  ...
  switch (pkt->opcode) {
    case ERT_START_DPU:
      return pkt->data + pkt->extra_cu_masks +
             (get_ert_dpu_data(pkt)->chained + 1) *
                 sizeof(struct ert_dpu_data) / sizeof(uint32_t);
...
static inline uint32_t* get_ert_regmap_end(struct ert_start_kernel_cmd* pkt) {
  // pkt->count includes the mandatory cumask which precededs data array
  return &pkt->cu_mask + pkt->count;
}

static inline uint64_t get_ert_regmap_size_bytes(
    struct ert_start_kernel_cmd* pkt) {
  return (get_ert_regmap_end(pkt) - get_ert_regmap_begin(pkt)) *
         sizeof(uint32_t);
}

#ifdef __linux__
#define P2ROUNDUP(x, align) (-(-(x) & -(align)))
static inline struct cu_cmd_state_timestamps* ert_start_kernel_timestamps(
    struct ert_start_kernel_cmd* pkt) {
  uint64_t offset = pkt->count * sizeof(uint32_t) + sizeof(pkt->header);
  /* Make sure the offset of timestamps are properly aligned. */
  return (struct cu_cmd_state_timestamps*)((char*)pkt +
                                           P2ROUNDUP(offset, sizeof(uint64_t)));
}

/* Return 0 if this pkt doesn't support timestamp or disabled */
static inline int get_size_with_timestamps_or_zero(struct ert_packet* pkt) {
  struct ert_start_kernel_cmd* skcmd;
  int size = 0;
