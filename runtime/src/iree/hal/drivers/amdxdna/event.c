...
typedef struct iree_hal_amdxdna_event_t {
  iree_hal_resource_t resource;
  iree_allocator_t host_allocator;
} iree_hal_amdxdna_event_t;

static const iree_hal_event_vtable_t iree_hal_amdxdna_event_vtable;

static iree_hal_amdxdna_event_t* iree_hal_amdxdna_event_cast(
    iree_hal_event_t* base_event) {
  IREE_HAL_ASSERT_TYPE(base_event, &iree_hal_amdxdna_event_vtable);
  return (iree_hal_amdxdna_event_t*)base_event;
}

static void iree_hal_amdxdna_event_destroy(iree_hal_event_t* base_event) {
  iree_hal_amdxdna_event_t* event = iree_hal_amdxdna_event_cast(base_event);
...
iree_status_t iree_hal_amdxdna_event_create(
  ...
  *out_event = NULL;
  ...
  iree_hal_amdxdna_event_t* event = NULL;
  iree_status_t status = iree_allocator_malloc(
      host_allocator, sizeof(*event), (void**)&event);
