...
int iree_hal_amdxdna_async_queue_worker_main(void* arg) {
  iree_hal_amdxdna_async_queue_t* queue = (iree_hal_amdxdna_async_queue_t*)arg;
  while (true) {
    ...
    iree_hal_amdxdna_async_op_t* head_op = (iree_hal_amdxdna_async_op_t*)head;
    ...
    while (op) {
      ...
      if (iree_status_is_ok(status) && !deferred && queue->frontier_tracker) {
        ...
        uint64_t epoch =
            (uint64_t)iree_atomic_fetch_add(queue->frontier_epoch, 1,
                                            iree_memory_order_acq_rel) +
            1;
