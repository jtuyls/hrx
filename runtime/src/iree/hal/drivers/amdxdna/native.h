...
// Resolves an accessible device's enumeration identity without opening it
// (filesystem / sysfs reads only, no ioctls), suitable for driver-level
// query_available_devices. Returns NOT_FOUND if no accessible device node is
// available.
//   - out_device_path: the stable device-node path (`options->device_path` if
//     set, otherwise the first /dev/accel/accel* node).
//   - out_arch: the NPU architecture name ("Phoenix"/"Strix"), or empty if it
